...
class ActiveLearningLocalForwardModelData:

    def __init__(self, mask, observation=None, initial_pattern_count=1000, max_size=100000):
        self.base_mask = mask
    ...
    def add_observation(self, prev_observation, action, observation):
        ...
        if len(self.known_patterns) < self.max_patterns:
            observed_pattern = extract_observed_patterns(prev_observation, self.padded_array, self.span, self.mask_array, self.observed_patterns, action, observation)

            for i, pattern in enumerate(self.observed_patterns):
                el = tuple(pattern)
                if el[:-1] not in self.known_patterns:
                    self.patterns[len(self.known_patterns), :] = pattern
                    self.known_patterns[el[:-1]] = el[-1]
                    if len(self.known_patterns) == self.max_patterns:
    ...
    def get_prediction(self, state, action):
        ...
        return np.array([self.known_patterns[tuple(x)] for x in patterns]).reshape(state.shape)
